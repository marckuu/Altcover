...
func (f *FavoritesService) GetFavoriteCovers(ctx context.Context, userID uuid.UUID) ([]domains.Cover, error) {
	// Получение ids избранных обложек пользователя с userID
	coversIDs, err := f.favoritesRepository.GetFavoriteCoversIDs(ctx, userID)
	if err != nil {
